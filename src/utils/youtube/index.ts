...
export const extractVideoIdFromUrl = (youtubeUrl: string): string | null => {
    const regex = /(?:youtu\.be\/|\/video\/|\/embed\/|\/v\/|\/e\/|\/shorts\/|watch\?v=|\/videos\/|\/uploads\/|%2Fvideos%2F)([^#&?\/\n]+?)(?:$|&|\?|#|\/)/;
...
export const checkVideoOwnership = async (accessToken: string, videoId: string | null): Promise<boolean> => {
    ...
    const userChannelResponse: AxiosResponse<ChannelResponse> = await axios.get('https://www.googleapis.com/youtube/v3/channels?part=id&mine=true', { headers });
    const userChannelId = userChannelResponse.data.items[0].id;

    // 2. Retrieve video details
    const videoResponse: AxiosResponse<VideoResponse> = await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${videoId}`, { headers });
