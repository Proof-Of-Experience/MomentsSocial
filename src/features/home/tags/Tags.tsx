      ...
      <div className="flex flex-row items-center justify-between">
        ...
        <div className="w-[75%]">
          <Slider ref={slider}  {...momentSliderSettings}>
            ...
            <button
              className={`${!tagParam ? ' border-b-[1px] border-[#00A1D4] text-[#00A1D4]' : 'text-[#7B7788]'} p-4 leading-trim font-inter font-[16px] text-base`}
              title="All"
              // disabled={!tagParam}
              onClick={() => onClick('all')}>
              All
            </button>
            {
              tags.map((item: any, index: number) => {
                ...
                return (
                  <button
                    key={`tag-${index}`}
                    className={`${tagParam == item.hashtag ? 'border-b-[1px] border-[#00A1D4] text-[#00A1D4]' : 'text-[#7B7788]'} p-4 leading-trim font-inter font-[16px] text-base`}
                    title={item.name}
