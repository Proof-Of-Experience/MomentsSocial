...
const ProfileWallet = ({ username, publicKey, userDetails }: any) => {
	...
	const fetchCreatorCoin = async () => {
		...
		const modifiedCoinsPurchased =
			creatorCoinData?.Hodlers.length > 0 &&
			creatorCoinData?.Hodlers.filter((item: any) => item.HasPurchased);
		setCoinsPurchased(modifiedCoinsPurchased);

		const modifiedCoinsReceived =
			creatorCoinData?.Hodlers.length > 0 &&
			creatorCoinData?.Hodlers.filter((item: any) => !item.HasPurchased);
				...
				<>
					<div className="grid grid-cols-4 gap-6">
															...
															<Tab.Panels className="mt-2">
																...
																<Tab.Panel className="text-center">
																	<button
																		...
																		onClick={() =>
																			window.open(
																				'https://exchange.coinbase.com/trade/DESO-USD',
																				'_blank'
																			)
																		}
																	>
																	...
																	<button
																		...
																		onClick={() =>
																			window.open(
																				'https://gate.io/trade/deso_usdt?ref=12312805',
																				'_blank'
																			)
																		}
																	>
																	...
																	<button
																		...
																		onClick={() =>
																			window.open(
																				'https://www.lbank.com/en-US/trade/deso_usdt',
																				'_blank'
																			)
																		}
																	>
																	...
																	<button
																		...
																		onClick={() =>
																			window.open(
																				'https://www.coinex.com/exchange/deso-usdt',
																				'_blank'
																			)
																		}
																	>
																	...
																	<button
																		...
																		onClick={() =>
																			window.open(
																				'https://www.ascendex.com/en/basic/cashtrade-spottrading/usdt/deso',
																				'_blank'
																			)
																		}
																	>
																	...
																	<button
																		...
																		onClick={() =>
																			window.open(
																				'https://www.jbex.com/exchange/DESO/USDT',
																				'_blank'
																			)
																		}
																	>
						...
						<div className="bg-gray-100 shadow rounded-lg px-5 py-6 col-span-1">
							...
							<Transition
								appear
								show={showByCoinsModal}
								as={Fragment}
							>
								<Dialog
									as="div"
									className="relative z-10"
									onClose={() => setShowByCoinsModal(false)}
								>
									<Transition.Child
										as={Fragment}
										enter="ease-out duration-300"
											...
											>
												<Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white px-6 py-8 my-5 text-left align-middle shadow-xl transition-all relative z-50">
													...
													<div className="mt-5">
														{isLoadedProfiles ? (
															<LoadingSpinner
																isLoading={isLoadedProfiles}
															/>
														) : (
															<table className="table-auto w-full">
																<thead>
																	<tr>
																		<th className="border-b border-black py-2 text-left">
																			Name
																		</th>
																		<th className="border-b border-black py-2 text-left">
																			Price
																						...
																						>
																							...
																							<span className="flex w-[150px] truncate">
																								{profileItem?.Username
																									? profileItem?.Username
																									: profileItem.PublicKeyBase58Check}
							...
							<Tab.Panels className="px-10 mt-2 mb-16">
								<Tab.Panel>
									<table className="table-auto w-full">
										...
										<tbody>
											{coinsPurchased?.length > 0 ? (
												coinsPurchased.map((holder: any, index: number) => (
													<tr key={index}>
														<td className="border-b py-3">
															<div
																className="flex items-center cursor-pointer"
																onClick={() =>
																	router.push(
																		`/user/${holder.ProfileEntryResponse?.Username}`
																	)
																}
															>
																<img
																...
																/>
																<span className="flex w-[150px] truncate">
																	{holder.ProfileEntryResponse
																		?.Username
																		? holder
																				.ProfileEntryResponse
																				?.Username
																		: holder.HODLerPublicKeyBase58Check}
																	{holder.ProfileEntryResponse
																		?.IsVerified && (
																		<CheckBadgeIcon className="ml-1 w-5 h-5 text-blue-500" />
																	)}
								...
								<Tab.Panel>
									<table className="table-auto w-full">
										...
										<tbody>
											{coinsReceived?.length > 0 ? (
												coinsReceived.map((holder: any, index: number) => (
													<tr key={index}>
														<td className="border-b py-3">
															<div
																className="flex items-center cursor-pointer"
