...
const ClassicEditor =
	typeof window !== 'undefined' ? require('@ckeditor/ckeditor5-build-classic') : null;
	...
	const renderPreviewVideo = () => {
		return (
			<>
				...
				{videoFile && (
					<p>
						<span className="font-semibold">File Name: </span> {videoFile.name}
					</p>
				)}
			</>
		);
			...
			<div className="flex flex-col items-center justify-center text-center relative mb-10 h-[330px]">
				...
				>
					<div className="flex flex-col justify-center items-center">
						<ArrowUpTrayIcon className="h-20 w-20 z-10 text-black" />
						<h4 className="my-5 text-3xl">Drag & drop a file to upload</h4>
						<h5 className="mb-5">
							Or <span className="text-blue-500">browse file </span>from device
						</h5>
					</div>
				</label>
			</div>
	...
	const renderProgressAndError = () => {
		return (
			<>
				...
				{progressFormatted && <p className="text-center">{progressFormatted}</p>}
			</>
		);
	};
